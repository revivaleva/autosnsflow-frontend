...
export default function AdminUsersPage() {
  ...
  const DASHBOARD_PATH = "/"; // もしダッシュボードが /dashboard ならここを変更してください

  // [ADD] 編集モーダル用
  const [editOpen, setEditOpen] = useState(false);
  const [target, setTarget] = useState<AdminUserRow | null>(null);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState<{ apiDailyLimit: number; autoPostAdminStop: boolean; autoPost: boolean }>({
    apiDailyLimit: 200,
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        {debugOpen && (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white rounded shadow-xl p-4 w-full max-w-3xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="absolute inset-0" onClick={() => setDebugOpen(false)} />
          </div>
        )}
              ...
              <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
                ...
                <div className="space-y-4">
                  <div className="text-sm">
                    ...
                    <div className="font-mono">{target.email}</div>
                  </div>
