...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // For demo: store under a synthetic user and account id (TODO: map real account)
    const userId = req.cookies['__session'] || `local-${crypto.randomUUID()}`;
    const accountId = `threads_${crypto.randomUUID().slice(0,8)}`;

    // Save token to ThreadsAccounts table
    const item = {
      PK: { S: `USER#${userId}` },
