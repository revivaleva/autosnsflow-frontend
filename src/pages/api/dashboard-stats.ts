...
const toNum = (v: any) =>
  typeof v === "number" ? v : v?.N ? Number(v.N) : 0;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // recentErrors base from posts/replies/accounts
    ...
    replies.forEach(r => {
      ...
      if (r.status?.S === "failed" && createdAt >= last7Start) {
        recentErrors.push({ type: "reply", id: r.SK?.S || "", at: createdAt, message: r.errorDetail?.S || "返信失敗" });
      }
    });
    ...
    accounts.forEach(a => {
      ...
      if (msg && createdAt >= last7Start) {
        recentErrors.push({ type: "account", id: a.SK?.S || "", at: createdAt, message: msg });
      }
    });
    ...
    try {
      ...
      logs.forEach(l => {
        try {
          ...
          try { const d = JSON.parse(l.detail?.S || '{}'); if (d && typeof d === 'object' && !message && d.message) message = String(d.message).slice(0,200); } catch(_){}
