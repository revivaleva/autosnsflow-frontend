...
export async function getTokenForAccount({ userId, accountId }) {
    if (!userId)
...
export async function deleteThreadsPostWithToken({ postId, token }) {
    if (!postId)
...
export async function deleteThreadsPost({ postId, accountId, userId }) {
