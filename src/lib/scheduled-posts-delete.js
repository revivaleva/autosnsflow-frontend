...
export async function deleteScheduledRecord({ userId, sk, physical = true }) {
    if (!userId || !sk)
        throw new Error('userId and sk required');
    ...
    try {
        ...
        if (physical) {
            ...
            while (attempt < maxAttempts) {
                try {
                    ...
                    try {
                        console.warn('[deleteScheduledRecord] physical delete failed (attempt ' + attempt + ')', { userId, sk, error: String(e) });
                    }
                    catch (_) { }
        ...
        try {
            console.warn('[deleteScheduledRecord] failed', { userId, sk, error: String(e) });
        }
        catch (_) { }
