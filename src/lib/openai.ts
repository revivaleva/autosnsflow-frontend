...
export async function callOpenAIText({ apiKey, model, systemPrompt, userPrompt, temperature, max_tokens }: any) {
  ...
  if (!primary.ok) {
    // if model access issue, attempt fallbacks
    try {
  ...
  // retry once with smaller max if inference returned empty
  if (!text && inference) {
    try {
      ...
      if (retryText) {
        text = retryText;
        return { text, raw: retry.data };
      }
    } catch (_) {}
  }

  // fallback to a smaller model if still empty
  if (!text && inference) {
    try {
