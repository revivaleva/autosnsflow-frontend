...
export const handler = async (event: any = {}) => {
  try {
    ...
    // 1) Get token item (plain text key)
    ...
    // 2) Overwrite bind: set bound_device_id = device_id, session_expires_at = now + LEASE
    const leaseEnd = now + LEASE_SECONDS;
    await ddb.send(new UpdateItemCommand({
      TableName: TOKENS_TABLE,
      Key: marshall({ token }),
      UpdateExpression: 'SET bound_device_id = :did, bound_at = :now, session_expires_at = :lease, updated_at = :now',
      ExpressionAttributeValues: marshall({ ':did': device_id, ':now': now, ':lease': leaseEnd }),
    }));

    // 3) Audit log (with TTL)
    try {
      const log = { event_id: `${token}#${now}#validate`, token, event_type: 'validate', actor: device_id, ts: now, detail: device_info || {}, ttl: now + LOG_TTL_SECONDS };
      await ddb.send(new PutItemCommand({ TableName: TOKEN_EVENTS_TABLE, Item: marshall(log) }));
    } catch (e) {
      // non-fatal
      console.warn('putLog failed', String(e));
    }

    // 4) Re-fetch minimal fields to return (or use item)
    ...
    return jsonResp(500, { ok: false, code: 'SERVER_ERROR', message: String(e?.message || e) });
  }
};
